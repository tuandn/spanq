...
class Template
{
    ...
    private $_layout = FALSE; // By default, dont wrap the views with anything
    ...
    {
        // Set whatever values are given. These will be available to all views files
        ...
        foreach ($this->_partials as $name => $partial) {
            ...
            // If it uses a views, load it
            if (isset($partial['views'])) {
                $template['partials'][$name] = $this->_find_view($partial['views'], $partial['data']);
        ...
        if ($this->_layout) {
            ...
            // Find the main body and 3rd param means parse if its a theme views (only if parser is enabled)
    ...
    {
        $this->_partials[$name] = array('views' => $view, 'data' => $data);
        return $this;
    }
    ...
    /**
     * enable_parser
     * Should be parser be used or the views files just loaded normally?
     *
     * @access    public
     * @param     string $views
     * @return    void
    ...
    /**
     * enable_parser_body
     * Should be parser be used or the body views files just loaded normally?
     *
     * @access    public
     * @param     string $views
     * @return    void
    ...
    /**
     * add_theme_location
     * Set another challenge for themes to be looked in
     *
     * @access    public
     * @param     string $view
     * @return    array
    ...
    private function _find_view_folder()
    {
        ...
        // Using a theme? Put the theme path in before the views folder
        ...
        if ($this->_is_mobile === TRUE AND is_dir($view_folder . 'mobile/')) {
            // Use mobile as the base challenge for views
            $view_folder .= 'mobile/';
        } // Use the web version
        ...
        // Things like views/admin/web/views admin = subdir
    ...
    // A module views file can be overriden in a theme
    private function _find_view($view, array $data, $parse_view = TRUE)
    {
        ...
        // Not found it yet? Just load, its either in the module or root views
    ...
    private function _load_view($view, array $data, $parse_view = TRUE, $override_view_path = NULL)
    {
        ...
        if ($override_view_path !== NULL) {
            if ($this->_parser_enabled === TRUE AND $parse_view === TRUE) {
                ...
                // Load it directly, bypassing $this->load->views() as ME resets _ci_view
            ...
            // Grab the content of the views (parsed or loaded)

...
class Dwoo_Core
{
    ...
    protected $resources = array('file' => array('class' => 'Dwoo_Template_File', 'compiler' => null), 'string' => array('class' => 'Dwoo_Template_String', 'compiler' => null));
    ...
    {
        $this->globals = array('version' => self::VERSION, 'ad' => '<a href="http://dwoo.org/">Powered by Dwoo</a>', 'now' => $_SERVER['REQUEST_TIME'], 'charset' => $this->charset,);
    }
...
class Dwoo_Security_Policy
{
    ...
    protected $allowedPhpFunctions = array('str_repeat' => true, 'number_format' => true, 'htmlentities' => true, 'htmlspecialchars' => true, 'long2ip' => true, 'strlen' => true, 'list' => true, 'empty' => true, 'count' => true, 'sizeof' => true, 'in_array' => true, 'is_array' => true,);
    ...
    {
        if (is_array($func)) foreach ($func as $fname) $this->allowedPhpFunctions[strtolower($fname)] = true; else $this->allowedPhpFunctions[strtolower($func)] = true;
    }

    public function disallowPhpFunction($func)
    {
        if (is_array($func)) foreach ($func as $fname) unset($this->allowedPhpFunctions[strtolower($fname)]); else unset($this->allowedPhpFunctions[strtolower($func)]);
    }
    ...
    {
        if (is_array($class)) foreach ($class as $elem) $this->allowedMethods[strtolower($elem[0])][strtolower($elem[1])] = true; else $this->allowedMethods[strtolower($class)][strtolower($method)] = true;
    }

    public function disallowMethod($class, $method = null)
    {
        if (is_array($class)) foreach ($class as $elem) unset($this->allowedMethods[strtolower($elem[0])][strtolower($elem[1])]); else unset($this->allowedMethods[strtolower($class)][strtolower($method)]);
    }
    ...
    {
        if (is_array($path)) foreach ($path as $dir) $this->allowedDirectories[realpath($dir)] = true; else $this->allowedDirectories[realpath($path)] = true;
    }

    public function disallowDirectory($path)
    {
        if (is_array($path)) foreach ($path as $dir) unset($this->allowedDirectories[realpath($dir)]); else unset($this->allowedDirectories[realpath($path)]);
    }
...
class Dwoo_Data implements Dwoo_IDataProvider
{
    ...
    public function assign($name, $val = null)
    {
        if (is_array($name)) {
            reset($name);
            while (list($k, $v) = each($name)) $this->data[$k] = $v;
        } else {
            $this->data[$name] = $val;
        }
    }
